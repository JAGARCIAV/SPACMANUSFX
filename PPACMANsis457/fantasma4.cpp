...
void Fantasma4::render()
{
	...
	nuevaTextura = SDL_CreateTextureFromSurface(renderer, fantasma4Surface);
	if (nuevaTextura == NULL)
	{
		cout << "No se puede crear una textura a partir de fantasma4Surface, SDL Error: " << SDL_GetError() << endl;
	}
	else
	{
		// Obtener dimension de la imagen
		ancho = fantasma4Surface->w;
		alto = fantasma4Surface->h;
	}
